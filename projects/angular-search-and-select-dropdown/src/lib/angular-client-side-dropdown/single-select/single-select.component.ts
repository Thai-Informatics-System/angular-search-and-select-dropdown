...
@Component({
  selector: 'angular-single-select',
...
export class SingleSelectComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['initialData']) {
      ...
      // console.log("=== angular-single-select :: ngOnChanges['initialData'] ===", this.initialData);
    ...
    if (changes['config']) {
      ...
      // console.log("=== angular-single-select :: ngOnChanges['config'] ===", this.config);
      this.getData();
    }
  }
  ...
  writeValue(value: string) {
    ...
    if (valueChanged) {
      // console.log("=== angular-single-select :: writeValue ===", value);
        ...
        if (this.badgeKey && this.badgeKey != '' && selectedValue[this.badgeKey]?.length) {
          ...
          selectedValue[this.badgeKey]?.map((badge: any) => {
            if (this.getBadge(badge)) {
              htmlStr += `<span class="angular-badge-sm angular-badge-round ${this.getBadge(badge)?.class}" style="font-size: 12px !important; padding: 2px 5px !important; line-height: 16px !important;">${this.getBadge(badge)?.value}</span>`;
            }
          });
          htmlStr += `</span>`;
        }
  ...
  setCustomClass() {
    ...
    const panelClass = `angular-dropdown-panel angular-client-side-single-select-panel ${this.panelClass}`;
  ...
  setSearchFieldWith() {
    const parent = document.querySelector('.angular-dropdown-panel') as HTMLElement;
